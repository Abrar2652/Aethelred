...
def load_ogbn_dataset(path_pkl, path_graph):
    """Obtain the mutagenicity dataset from text files.
    ...
    :returns: adjecency matrix, node features, labels.
    """
    with open(path_graph, 'rb') as fin:
    ...
    for i in range(edge_index.shape[1]):
        if edge_index[0][i]>edge_index[1][i]:
            ...
            r+=1
        if (edge_index[0][i])<30000 and (edge_index[1][i])<30000:
            if len(new_index)==0:                
                new_index.append([int(maps[edge_index[0][i]])])
                new_index.append([int(maps[edge_index[1][i]])])
            else:
                new_index[0].append(int(maps[edge_index[0][i]]))
                new_index[1].append(int(maps[edge_index[1][i]]))
    new_index = np.array(new_index)
    with open(path_pkl,'wb') as fout:
        pkl.dump((new_index, features[indices],labels[indices]),fout,protocol = 4)
    return new_index, features, labels

def get_graph_data_2(path):
    """Obtain the mutagenicity dataset from text files.
...
def load_real_dataset_2(path_pkl, path_graph):
    """Obtain the mutagenicity dataset from text files.
    ...
    :returns: adjecency matrix, node features, labels.
    """
    edge_lists, graph_labels, edge_label_lists, node_label_lists = get_graph_data_2(path_graph)
    graph_labels[graph_labels == -1] = 0

    max_node_nmb = np.max([len(node_label) for node_label in node_label_lists]) + 1  # add nodes for each graph

    edge_label_nmb =1#= np.max([np.max(l) for l in edge_label_lists]) + 1
    node_label_nmb = 14#np.max([np.max(l) for l in node_label_lists]) + 1
