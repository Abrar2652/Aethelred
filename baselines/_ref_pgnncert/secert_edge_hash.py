# -*- coding: utf-8 -*-
"""
SECert-E: Shared-Encoder Certified Defense (Edge-Centric Variant)

Key innovation over PGNNCert-E:
  1. SHARED GNN backbone across all S subgraph classifiers
     - Backbone receives gradients from ALL S subgraphs → richer representations
     - ~S× fewer parameters than S independent models (backbone shared, only heads differ)
     - Maintains deterministic certification (test-time: each perturbation affects ≤1 vote)

  2. Margin-boosting training loss
     - Hinge loss on vote margin directly optimizes for larger certified perturbation radius
     - L_total = L_ce + lambda_margin * L_margin

  3. Structural positional encoding (optional)
     - Append perturbation-invariant positional features to compensate for sparse subgraphs
"""
...
class HashAgent:
    """Edge-centric hash agent — identical to PGNNCert-E for fair comparison."""
...
class SECertNodeClassifier(nn.Module):
    """
    SECert Edge-Centric Node Classifier.

    Architecture:
      - 1 shared GNN backbone (conv layers)
      - T lightweight per-subgraph classification heads
      - Margin-boosting training objective

    Certification guarantee (Theorem):
      At test time, parameters are frozen. Each edge perturbation changes the
      input to the backbone for exactly 1 subgraph. Other subgraphs' outputs
      are unaffected. Therefore each perturbation changes at most 1 vote,
      and the same certification bound as PGNNCert applies.
    """
    ...
    def __init__(self, Hasher, edge_index, x, y, train_mask, val_mask, test_mask,
        ...
        # Per-subgraph classification heads
    ...
    def _compute_margin_loss(self, votes, true_labels):
        """Hinge loss on vote margin — directly optimizes certified perturbation radius."""
        batch_size = votes.shape[0]
        if batch_size == 0:
            return torch.tensor(0.0, device=self.device)

        # Get top-1 vote count for true class
        true_votes = votes[torch.arange(batch_size), true_labels]
        # Mask true class and get runner-up
        masked_votes = votes.clone()
        masked_votes[torch.arange(batch_size), true_labels] = -1e9
        runner_up_votes = masked_votes.max(dim=1)[0]
        # Hinge loss: want margin > 2 (certified against at least 1 perturbation)
    ...
    def train_model(self, train_args):
        ...
        for epoch in range(train_args["epochs"]):
            ...
            # Forward through shared backbone + per-subgraph heads
            all_preds = []
            ...
            if self.lambda_margin > 0 and len(all_preds) > 0:
                ...
                loss_margin = self._compute_margin_loss(
                    soft_votes, self.y[self.train_mask_device]
                )
...
class SECertGraphClassifier(nn.Module):
    """
    SECert Edge-Centric Graph Classifier.

    Same shared-backbone architecture as the node classifier,
    adapted for graph-level classification tasks.
    """
