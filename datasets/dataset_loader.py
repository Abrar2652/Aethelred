...
# --------------------------------------------------------------------------
# Explanation ground-truth datasets (for Table: faithful explanation eval)
# --------------------------------------------------------------------------

def _load_ba_shapes(seed=42):
    """
    BA-Shapes: single BA graph + planted house motifs, 4-class node classification.

    Node feature: 10-dim constant (ones) — standard in the literature.
    GT edge mask: data.edge_mask from ExplainerDataset (1 = motif edge).
    Classes: 0=BA node, 1/2/3=house positions (top/middle/bottom).
    """
    ...
    n = data.num_nodes
    data.x = torch.ones(n, 10, dtype=torch.float32)
    ...
    for c in range(4):
        ...
        perm = torch.tensor(rng.permutation(idx.numel()), dtype=torch.long)
        idx = idx[perm]
        n_tr = int(idx.numel() * 0.6)
        n_vl = int(idx.numel() * 0.2)
        data.train_mask[idx[:n_tr]]           = True
        data.val_mask[idx[n_tr:n_tr + n_vl]]  = True
        data.test_mask[idx[n_tr + n_vl:]]     = True
...
def _load_tree_cycles(seed=42):
    """
    Tree-Cycles: binary tree (depth=8) + planted 6-cycles, 2-class node classification.

    Node feature: 10-dim constant (ones).
    GT edge mask: data.edge_mask from ExplainerDataset (1 = cycle edge).
    Classes: 0=tree node, 1=cycle node.
    """
    from torch_geometric.datasets import ExplainerDataset
...
def _mutag_gt_edge_masks(graphs):
    """
    Annotate each MUTAG graph with a ground-truth edge mask.

    GT = edges incident to a NO2-nitrogen: a nitrogen atom (col 1) that has
    at least one oxygen neighbor (col 2).  This excludes NH2 nitrogens whose
    hydrogen neighbours are implicit and cannot be seen in the graph, making
    the GT structurally non-trivial: correctly identifying it requires
    neighbourhood reasoning, not just reading a single node's feature vector.

    Node features: 7-dim one-hot [C=0, N=1, O=2, F=3, I=4, Cl=5, Br=6].

    Graphs with no NO2 group get an all-zero mask; the NaN guards in _auc /
    _preck will exclude them from the reported mean automatically.

    Adds g.edge_mask (float32, shape [E]) to every graph in-place.
    """
    for g in graphs:
        is_n = g.x[:, 1] > 0.5   # nitrogen atoms
        is_o = g.x[:, 2] > 0.5   # oxygen atoms
        src, dst = g.edge_index[0], g.edge_index[1]

        # nitrogen nodes that have ≥1 oxygen neighbour (= NO2 nitrogen)
        no2_n = torch.zeros(g.num_nodes, dtype=torch.bool, device=g.x.device)
        no2_n[src[is_n[src] & is_o[dst]]] = True
        no2_n[dst[is_n[dst] & is_o[src]]] = True

        # GT edge: at least one endpoint is a NO2 nitrogen
        g.edge_mask = (no2_n[src] | no2_n[dst]).float()
    return graphs
